...
#
    ...
    def to_LinkedList(self):                      # Compulsory
        #lst = LinkedList()
        #lst_c = self.to_list
        #lst.first = lst.Node(lst_c[0], n)
        #for i in range(len(lst_c)):
        #    if i == 0:
        #        n = lst.Node(lst_c[-1], None)
        #    elif i < len(lst_c): 
        #        n = lst.Node(lst_c[i], n)
        #    
#
        #def _to_LinkedList(lst, x=None):
        #    for i in reversed(self.root):
        #        y = lst.Node(i, x)
        #        x = i
        #    return 
#
        #lst.first = _to_LinkedList(lst)
        #
        #return lst
        
        #lst = LinkedList()
        #lst_copy = self.to_list()
        #n = self.root
        #def _to_LinkedList(lst_copy):
        #    if len(lst_copy) == 1:
        #        return lst.Node(lst_copy[0], None)
        #    else:
        #        return lst.Node(lst_copy[0], _to_LinkedList(lst_copy[1:]))
#
        #lst.first = _to_LinkedList(lst_copy)
#
        #return lst
       ...
       # res = LinkedList()
       # it = iter(self)
       # res.first = res.Node(next(it), None)
       # f = res.first
       # for i in it:
       #     f.succ = res.Node(i, None)
       #     f = f.succ
       # return res
    ...
    def ipl(self):                                # Compulsory
        ...
        def _ipl(n, int):
            if n == None:
                ...
                return _ipl(n.left, int + 1) + _ipl(n.right, int + 1) + int
...
def main():
    ...
    print('Övn 20')
    for k in [5000, 10000, 20000, 100000]:
